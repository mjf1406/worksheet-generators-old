...
document.addEventListener("DOMContentLoaded", function () {
    ...
    toggleButtons.forEach((btn) => {
        btn.addEventListener("click", function () {
            ...
            if (modalId === "save-preset-modal") {
                let selectedPreset =
                    localStorage.getItem("clicked-preset") ==
                    "word-search-presets"
                        ? document.getElementById("presets-dropdown").value
                        : document.getElementById("presets-dropdown-crossword")
                              .value;
                if (selectedPreset !== "load") {
                    ...
                    let displayPresetName =
                        document.getElementById("update-preset-name");
                ...
                if (selectedPreset === "load") {
                    return makeToast(
                        "Please select a preset before deleting!",
                        "warning"
                    );
                } else {
                    let displayPresetName = document.getElementById(
    ...
    deletePreset.addEventListener("click", function () {
        ...
        let selectedPreset =
            preset === "word-search-presets"
                ? document.getElementById("presets-dropdown").value
                : document.getElementById("presets-dropdown-crossword").value;
    ...
    for (let x = 0; x < height; x++) {
        for (let y = 0; y < width; y++) {
            ...
            if (answerKey.length > 0)
                isWordCoord = answerKey[x][y] ? true : false;
